...
func_prepRevSport <- function(start_d, end_d) {
  ...
  coaching.consol_full <- tran.revsport %>% 
    left_join(revsport.payments, by = c("transfer_id")) %>% 
  ...
  coaching.consol <- coaching.consol_full %>% 
    filter(rpt_date >= start_d & rpt_date <= end_d) %>% 
    select(-transfer_ref)
  ...
  refunds.consol <- tran.revsport %>% 
    ...
    left_join(coaching.consol_full %>% 
...
bal.cb <- bal.ob %>% 
  select(rpt_date, map_cat, cum_amt) %>%
  rename(amt = cum_amt) %>% 
  # Append the changes
  bind_rows(bs.data) %>% 
...
gt(data = tbl_pl
  ...
  tab_header(
    ...
    , subtitle = md("*For the period 1 April 2022 to 31 March 2023*")) %>% 
...
gt(data = tbl_bs
  ...
  cols_label(Opening_Balance = rpt.open - 1
             , Closing_Balance = rpt.curr) %>% 
  tab_style(style = list(
    cell_text(weight = "bold")
    , cell_borders(color = "#002A52")
    )
...
func_plotTornado(inputData = rpt.monthly.data
               ...
               , inputLimits = c(-8000, 8000))

```

#### Year to date performance
```{r ytdPL, fig.width=9, fig.height=10, echo=FALSE, warning=FALSE, message=FALSE, eval=TRUE}
...
#### Monthly performance
```{r rptMonthByMonth, fig.width=9, fig.height=6, echo=FALSE, eval=TRUE, warning=FALSE, message=FALSE, eval=FALSE}
...
loop <- 1
for (i in 19:as.integer(str_sub(rpt.fy, 3, 4))) {
  
  perd <- paste0("FY", i)
  hist.data <- read_csv(paste0(perd, "/PROC/rpt_consol.csv"))
  
  if (loop == 1) {
    hist.consol <- hist.data
  } else {
    hist.consol <- hist.data %>% 
      bind_rows(hist.consol)
  }
...
ggplot(hist.trends, aes(x = rpt_perd, y = amt, fill = fs_cat)) +
  geom_bar(stat = "identity", show.legend = F) +
...
```{r plotCashBalance, fig.width=9, fig.height=5, echo=FALSE, warning=FALSE, message=FALSE, eval=FALSE}
...
#### Junior Program
```{r junior_detail, echo=FALSE, warning=FALSE, message=FALSE, eval=FALSE}
...
#### Squad Program
```{r squad_detail, echo=FALSE, warning=FALSE, message=FALSE, eval=FALSE}
...
```{r admin_detail, echo=FALSE, warning=FALSE, message=FALSE, eval=FALSE}
